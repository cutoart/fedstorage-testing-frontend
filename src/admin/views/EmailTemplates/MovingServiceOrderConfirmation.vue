    ...
    <CCol>
      <CCard>
        <CForm @submit.prevent="handleSubmit">
          <CCardHeader>
            <strong>MovingService Order Confirmation Html</strong>
          </CCardHeader>
          <CCardBody>
            <CRow>
              <CCol sm="6">
...
export default {
  ...
  created() {
    ...
    document.title =
      this.$store.state.projecttitle + " - MovingService Order Confirmation Html";
    APIService.post("/GetEmailTemplateByType", {
      type: "MovingServiceOrderConfirmation",
    }).then((response) => {
  ...
  methods: {
    handleSubmit() {
      ...
      this.vmodel.type = "MovingServiceOrderConfirmation";
      APIService.post("/AddUpdateEmailTemplate", _vmodel).then(() => {
        this.$router.go({ name: "MovingService Order Confirmation" });
