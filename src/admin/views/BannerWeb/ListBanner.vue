    ...
    <CCard>
      <CCardHeader>
        <CRow>
          ...
          <CCol col="2">
            <CButton block router-link :to="{ name: 'AddBannerWeb' }" color="dark"
              >Add Banner</CButton
            >
          </CCol>
        </CRow>
...
export default {
  ...
  methods: {
    getBannerListData() {
      APIService.post("/ListOfBannerWeb", { usertype: "admin" }).then(
        (response) => {
    ...
    deActivateItem(id, deleted) {
      ...
      this.$confirm({
        ...
        callback: (confirm) => {
          if (confirm) {
            APIService.delete("/DeActivateBannerWeb", req).then((response) => {
              this.$set(this, "item", response.data);
            });
          }
        },
    ...
    deleteItem(id) {
      ...
      this.$confirm({
        ...
        callback: (confirm) => {
          if (confirm) {
            APIService.delete("/DeleteBannerWeb", req).then((response) => {
              this.$set(this, "item", response.data);
            });
          }
        },
