    ...
    <CCol>
      <CCard>
        <CForm @submit.prevent="handleSubmit">
          <CCardHeader>
            <strong>Add Mobile Banner</strong>
          </CCardHeader>
          <CCardBody>
            <CRow>
              <CCol sm="12">
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      if (this.id != undefined && this.id != null && this.id != "") {
        APIService.post("/UpdateBannerWeb", _vmodel, this.id).then(() => {
          this.$successAlert("Your data is updated.");
          this.$router.push({ name: "BannerWebList" });
        });
      } else {
        APIService.post("/AddBannerWeb", this.vmodel).then(() => {
